...
void printUsage(const char* argv0) {
  std::fprintf(
      ...
      "  --yaw-range-deg DEG         鼠标横向半幅 yaw，默认 20\n"
      "  --pitch-range-deg DEG       鼠标纵向半幅 pitch，默认 15\n"
      "  --hz N                      下发频率，默认 100\n"
      "  --distance M                下发距离，默认 1.0\n"
      "  --max-rate-rad-s N          速度限幅，默认 5\n"
      "  --max-acc-rad-s2 N          加速度限幅，默认 80\n"
      "  --no-feedback               不读取下位机反馈\n"
      "按键: q/Esc 退出，Space/p 暂停，r 回中，WASD/IJKL/方向键移动。\n"
      "说明: 串口收发单位固定为 rad / rad/s / rad/s²，窗口显示为度制。\n",
      argv0);
}
...
bool parseArgs(int argc, char** argv, Options& options) {
  ...
  if (options.baudrate <= 0 || options.hz <= 0.0 ||
    ...
    std::fprintf(stderr, "baudrate、hz 和角度范围必须大于 0\n");
    return false;
  }
  return true;
}
...
void nudgeInput(MouseState& state, const Options& options,
  ...
  const double x_delta = yaw_delta_deg / options.yaw_range_deg * (kWindowWidth / 2.0);
  const double y_delta = -pitch_delta_deg / options.pitch_range_deg * (kWindowHeight / 2.0);
  state.x = std::clamp(static_cast<int>(std::lround(state.x + x_delta)), 0, kWindowWidth - 1);
  state.y = std::clamp(static_cast<int>(std::lround(state.y + y_delta)), 0, kWindowHeight - 1);
}

CommandState mouseToCommand(const MouseState& mouse, const Options& options,
                            double dt, const CommandState& previous) {
  const double nx = (static_cast<double>(mouse.x) - kWindowWidth / 2.0) / (kWindowWidth / 2.0);
  const double ny = (static_cast<double>(mouse.y) - kWindowHeight / 2.0) / (kWindowHeight / 2.0);
  CommandState command;
  command.yaw = nx * options.yaw_range_deg * kDegToRad;
  command.pitch = -ny * options.pitch_range_deg * kDegToRad;
  if (dt > 1e-6) {
    command.yaw_vel = clampAbs((command.yaw - previous.yaw) / dt, options.max_rate_rad_s);
    command.pitch_vel = clampAbs((command.pitch - previous.pitch) / dt, options.max_rate_rad_s);
  }
  return command;
}
...
void drawWindow(const MouseState& mouse, const Options& options,
  ...
  cv::line(image, cv::Point(center.x, 0), cv::Point(center.x, kWindowHeight), cv::Scalar(70, 70, 70), 1);
  cv::line(image, cv::Point(0, center.y), cv::Point(kWindowWidth, center.y), cv::Scalar(70, 70, 70), 1);
  cv::rectangle(image, cv::Rect(1, 1, kWindowWidth - 2, kWindowHeight - 2), cv::Scalar(90, 90, 90), 1);
  const cv::Scalar point_color = mouse.paused ? cv::Scalar(60, 60, 220) : cv::Scalar(80, 220, 120);
  ...
  int y = 30;
  char line[256];
  std::snprintf(line, sizeof(line), "manual_gimbal_test  port=%s  baud=%d", options.port.c_str(), options.baudrate);
  ...
  std::snprintf(line, sizeof(line), "cmd yaw=%.2fdeg pitch=%.2fdeg dist=%.2fm mode=%d",
                command.yaw * kRadToDeg, command.pitch * kRadToDeg,
                command.distance, static_cast<int>(command.mode));
  drawText(image, y, line, mouse.paused ? cv::Scalar(80, 80, 250) : cv::Scalar(80, 240, 150));
  std::snprintf(line, sizeof(line), "vel yaw=%.1fdeg/s pitch=%.1fdeg/s  acc yaw=%.1fdeg/s2 pitch=%.1fdeg/s2",
                command.yaw_vel * kRadToDeg, command.pitch_vel * kRadToDeg,
                yaw_acc * kRadToDeg, pitch_acc * kRadToDeg);
  drawText(image, y, line, cv::Scalar(140, 220, 255));
  if (have_feedback) {
    std::snprintf(line, sizeof(line), "feedback mode=%u yaw=%.2fdeg pitch=%.2fdeg roll=%.2fdeg",
  ...
  drawText(image, y, "串口实际单位: rad / rad/s / rad/s²；q/Esc退出，Space暂停，r回中");
...
int main(int argc, char** argv) {
  ...
  serial_config.feedback_angles_in_degrees = false;
  ...
  try {
    ...
    std::fprintf(stderr, "OpenCV 窗口创建失败: %s\n", error.what());
    return 1;
  }

  std::printf("手动云台测试已打开: %s @ %d，TX=infantry_32，RX=infantry，单位=rad\n",
  ...
  while (running) {
    ...
    yaw_acc = clampAbs((command_state.yaw_vel - previous_yaw_vel) / dt, options.max_acc_rad_s2);
    pitch_acc = clampAbs((command_state.pitch_vel - previous_pitch_vel) / dt, options.max_acc_rad_s2);
    ...
    command.mode = mouse.paused ? hfut::GimbalMode::no_valid_measurement : hfut::GimbalMode::normal_measurement;

    if (!serial.sendCommand(command)) {
      std::fprintf(stderr, "串口发送失败: %s\n", serial.errorMessage().c_str());
    }
    ...
    if (now - last_print > std::chrono::milliseconds(500)) {
      std::printf("cmd yaw=%.2fdeg pitch=%.2fdeg vel=(%.1f,%.1f)deg/s acc=(%.1f,%.1f)deg/s²\n",
                  command.yaw * kRadToDeg, command.pitch * kRadToDeg,
                  command.yaw_vel * kRadToDeg, command.pitch_vel * kRadToDeg,
                  yaw_acc * kRadToDeg, pitch_acc * kRadToDeg);
      last_print = now;
