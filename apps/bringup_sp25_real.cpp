...
struct Options {
  ...
  std::string sp25_config{"configs/sp25_real.yaml"};
...
Options parseOptions(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--help" || arg == "-h") {
      std::printf(
          "Usage: bringup_sp25_real [options]\n"
          "  --hardware-config PATH   硬件 YAML，默认 configs/hardware.yaml\n"
          "  --sp25-config PATH       SP25 算法 YAML，默认 configs/sp25_real.yaml\n"
...
int run(const Options& options) {
  ...
  if (options.web_view) {
    ...
    std::printf("[bringup_sp25] web_view=%s stream=/stream.mjpg status=/status.json\n",
                web_server->url().c_str());
  }

  std::printf(
      "[bringup_sp25] camera=%s serial=%s:%s@%d dry_run=%s fire=%s enemy=%s bullet=%.2f\n",
  ...
  std::printf("[bringup_sp25] sp25_config=%s runtime_config=%s calibration_mode=%s\n",
  ...
  while (!g_stop.load() && (options.max_frames < 0 || processed < options.max_frames)) {
    ...
    if (!feedback_ready) {
      ...
      if (now - last_feedback_wait_log > std::chrono::seconds(1)) {
        std::printf("[bringup_sp25] 等待串口反馈 %s last_age=%lldms\n",
    ...
    if (!camera->read(frame, std::chrono::milliseconds(200))) {
      std::fprintf(stderr, "[bringup_sp25] 相机读帧超时: %s\n", camera->errorMessage().c_str());
      continue;
    }
    ...
    if (visual_end - last_log > std::chrono::seconds(1)) {
      std::printf(
          "[bringup_sp25] frames=%llu fps=%.1f armors=%zu tracked=%zu state=%s "
      ...
      std::printf(
          "[bringup_sp25] timing_ms total=%.1f serial_rx=%.2f capture=%.2f "
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    std::fprintf(stderr, "[bringup_sp25] fatal: %s\n", error.what());
    return 1;
  }
}
