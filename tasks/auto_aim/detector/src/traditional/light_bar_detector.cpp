...
//
// ROS-free port of the traditional light-bar detector. Input is BGR8 (the
// bridge wire format); the ROS2 original expected RGB, so the grayscale
// conversion and the red/blue channel votes are adjusted accordingly.
...
LightBarDetector::LightBarDetector(const int &bin_thres,
                                   const EnemyColor &color,
                                   const LightParams &l,
                                   const ArmorParams &a)
: binary_thres(bin_thres), detect_color(color), light_params(l), armor_params(a) {}

std::vector<Armor> LightBarDetector::detect(const cv::Mat &input) noexcept {
...
cv::Mat LightBarDetector::preprocessImage(const cv::Mat &bgr_img) noexcept {
  cv::cvtColor(bgr_img, gray_img_, cv::COLOR_BGR2GRAY);
...
std::vector<Light> LightBarDetector::findLights(const cv::Mat &bgr_img,
  ...
  for (const auto &contour : contours) {
    ...
    if (isLight(light)) {
      ...
      for (const auto &point : contour) {
        // BGR8 wire format: channel 0 is blue, channel 2 is red.
        sum_b += bgr_img.at<cv::Vec3b>(point.y, point.x)[0];
        sum_r += bgr_img.at<cv::Vec3b>(point.y, point.x)[2];
      }
...
bool LightBarDetector::isLight(const Light &light) noexcept {
...
std::vector<Armor> LightBarDetector::matchLights(const std::vector<Light> &lights) noexcept {
...
bool LightBarDetector::containLight(const int i, const int j, const std::vector<Light> &lights) noexcept {
...
ArmorType LightBarDetector::isArmor(const Light &light_1, const Light &light_2) noexcept {
...
cv::Mat LightBarDetector::getAllNumbersImage() const noexcept {
  if (armors_.empty()) {
...
void LightBarDetector::drawResults(cv::Mat &img) const noexcept {
  // Draw armors
