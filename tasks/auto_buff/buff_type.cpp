...
FanBlade::FanBlade(
  const std::vector<cv::Point2f> & kpt, cv::Point2f keypoints_center, FanBlade_type t,
  int class_id_in, float confidence_in)
: center(keypoints_center), type(t), class_id(class_id_in), confidence(confidence_in)
{
  points.insert(points.end(), kpt.begin(), kpt.end());
}
...
: r_center(center), light_num(ts.size())
{
  ...
  auto explicit_target_it = std::find_if(ts.begin(), ts.end(), [](const FanBlade & blade) {
    return blade.type == _target;
  });
  if (explicit_target_it != ts.end()) {
    std::iter_swap(ts.begin(), explicit_target_it);
  }
