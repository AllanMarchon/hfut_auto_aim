...
YOLO11_BUFF::YOLO11_BUFF(const std::string & config)
{
  ...
  device_ = yaml["device"] ? yaml["device"].as<std::string>() : device_;
  confidence_threshold_ = yaml["confidence"] ? yaml["confidence"].as<float>() : confidence_threshold_;
  iou_threshold_ = yaml["iou_threshold"] ? yaml["iou_threshold"].as<float>() : iou_threshold_;
...
std::vector<YOLO11_BUFF::Object> YOLO11_BUFF::get_multicandidateboxes(cv::Mat & image)
{
  ...
  cv::dnn::NMSBoxes(boxes, confidences, confidence_threshold_, iou_threshold_, indexes);
